...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        first_part = scheme_eval(first, env)
        mapped_part = rest.map(lambda f: scheme_eval(f, env))
        combined_part = scheme_apply(first_part, mapped_part, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambdas_v = Pair(target.rest, expressions.rest)
        env.define(target.first, do_lambda_form(lambdas_v, env))
        target_f = target.first
        return target_f
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    return LambdaProcedure(formals, rest, env)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    while expressions!=nil:
    	...
    	if is_false_primitive(last_val):
    		return last_val
...
def do_or_form(expressions, env):
    ...
    if expressions==nil:
    	return False
    while expressions!=nil:
    	eval_v = scheme_eval(expressions.first, env)
    	if is_true_primitive(eval_v):
    		return eval_v
    	expressions = expressions.rest
    return False
    # END PROBLEM 12
